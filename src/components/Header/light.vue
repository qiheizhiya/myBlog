<template>
  <div id="lampadario" @click="changebg">
    <input type="radio" name="switch" value="on" :checked="!dayTime"/>
    <input type="radio" name="switch" value="off" :checked="dayTime" />
...
export default {
  ...
  data () {
    return {
      dayTime: this.$store.state.isDayTime
    }
  },
  methods:{
    changebg() {
      this.dayTime === true ? this.dayTime = false : this.dayTime = true
      this.$emit('changeBackground',this.dayTime)
    }
  }
}
</script>
